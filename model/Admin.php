...
class Admin_Model extends Model {
	...
	public function getByname($name) {
		$rs = $this->db->table('#@_admin')->where("name = '$name'")->getOne();
		return $rs;	
	}
	
	public function getByUid($uid) {
		$rs = $this->db->table('#@_admin')->where("uid = $uid")->getOne();
		return $rs;	
	}
	...
	public function set($uid, $data) {
		$this->db->table('#@_admin')->where("uid = $uid")->update($data);
		return true;	
	}
	
	public function del($uid) {
		$this->db->table('#@_admin')->where("uid = $uid")->delete();
		return true;	
	}
